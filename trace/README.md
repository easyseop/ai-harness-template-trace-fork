...
```text
Claude가 각 단계에서 어떤 파일과 어떤 Rule ID를 근거로 판단했는지
사람이 나중에 확인할 수 있게 남긴다.
```

중요한 점은, 이 trace가 Claude의 내부 생각을 들여다보는 기능은 아니라는 것입니다. 대신 외부에서 확인 가능한 파일, Rule ID, 해시, evidence 연결 관계를 남깁니다.
...
```text
1. 지시 계층
   CLAUDE.md, templates/CLAUDE.md.hbs, commands/*.md
   Claude에게 trace를 출력하고 기록하라고 지시합니다.
...
| `trace/mark-loaded-file.sh` | `loaded`를 만듭니다. Claude가 읽었다고 보고한 파일의 경로, sha256, 크기, 수정 시간, Rule ID를 기록합니다. |
...
## Claude에게 지시를 내리는 Markdown 파일

아래 md 파일들은 Claude가 trace를 언제, 어떻게 남겨야 하는지 지시합니다.
...
loaded
  Claude가 실제로 읽었다고 기록한 파일

evidence
  Claude가 최종 판단 근거로 제시한 file#RULE-ID
...
`selected`는 Claude가 실제로 읽었다는 뜻이 아닙니다. 이 단계에서 참고 후보가 되어야 할 파일을 `record-runtime-trace.sh`가 고른 목록입니다.
...
1. 해당 command 파일
   ...
   설치된 프로젝트면 .claude/commands/run.md
...
이 섹션은 Claude가 실제 작업을 수행할 때, AI-facing Trace와 Runtime Trace가 어떤 순서로 연결되는지 설명합니다.
...
즉, AI-facing Trace는 가능하면 Runtime Trace를 기반으로 출력되어야 합니다. Runtime Trace가 없거나 검증에 실패했다면, Claude는 그 사실을 사용자에게 알려야 하며 검증된 것처럼 말하면 안 됩니다.
...
이때 Claude Code는 해당 command md를 읽고 수행합니다.
...
이 명령은 Claude가 이미 읽은 파일을 기록하는 명령이 아닙니다.
...
AI-facing Trace는 이 정보를 사용해 “현재 단계와 참고 후보 파일”을 설명할 수 있습니다. 하지만 이 시점의 신뢰도는 `selected_only`입니다. 즉, 아직 Claude가 실제로 읽었다고 기록한 단계는 아닙니다.

### 3단계. Claude가 command/reference/agent 파일을 읽음

Claude는 command md의 지시에 따라 필요한 파일을 읽습니다.
...
Claude가 파일을 읽었다고 판단하면, 해당 파일을 loaded로 남겨야 합니다.
...
Harness Trace는 Claude의 숨은 사고 과정을 보여준다고 주장하지 않습니다. 대신 외부에서 확인 가능한 증거를 연결합니다.
...
loaded_files_recorded
  ...
  selected_only보다 강하지만, Claude 내부 Read hook은 아닙니다.
...
`loaded`는 자동 OS hook이 아닙니다. 즉, Claude가 파일을 여는 순간을 운영체제 수준에서 감청하는 구조는 아닙니다.
...
따라서 `loaded`는 “Claude가 실제로 읽었다고 보고했고, 그 시점의 파일 내용이 무엇인지 검증 가능하다”는 의미입니다.

이것만으로 Claude의 내부 이해를 증명할 수는 없습니다. 그래서 최종 신뢰도는 `applied` 검증과 replay/gate/evaluate 결과를 함께 봐야 합니다.
...
```bash
.harness/trace/mark-loaded-file.sh --path .claude/commands/run.md
.harness/trace/mark-loaded-file.sh --path ARCHITECTURE_INVARIANTS.md
```
...
Claude가 `RULE-EVAL-PERSONA-001`만 인용했을 때, 정확히 1번을 근거로 삼았는지 2번을 근거로 삼았는지 알기 어렵습니다.
