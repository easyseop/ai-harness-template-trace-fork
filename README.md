# AI Harness Template with Trace & Replay Test

> 한국어 · [English original/reference](./README.en.md)

이 레포는 원본 `ai-harness-template`에 **Trace**와 **Replay Test**를 더한 팀용 하네스 템플릿입니다.
...
중요한 점은 `commands/*.md`가 실행 파일이 아니라는 것입니다. 이 파일들은 Claude Code가 읽고 따르는 작업 지시문입니다.
...
| 종류 | 설명 | 위치 |
|------|------|------|
| AI-facing Trace | Claude가 사용자에게 보여주는 설명용 trace | 대화 화면 |
...
1. 사용자가 /run 같은 command 실행
        ↓
2. Claude가 해당 command md를 읽음
        ↓
3. record-runtime-trace.sh 실행
   이 단계에서 참고 후보 파일을 selected로 기록
        ↓
4. Claude가 command, reference, agent 파일을 읽음
        ↓
...
```text
1. 해당 command 파일
   예: /run이면 commands/run.md, 설치된 프로젝트면 .claude/commands/run.md
...
| 파일 | 역할 |
|------|------|
| `CLAUDE.md` | 이 템플릿 레포에서 trace 출력과 검증 원칙 정의 |
| `templates/CLAUDE.md.hbs` | 새 프로젝트에 설치될 `CLAUDE.md` 템플릿 |
...
```text
commands/run.md 또는 .claude/commands/run.md
...
하네스 설계를 검증할 때는 `claude_mediated` 흐름을 권장합니다.
...
이 명령은 먼저 `replay_prompt.md`를 만듭니다. 그 다음 Claude Code가 그 prompt와 target command md, fixture를 읽고 실제 `actual_trace.md`, `actual_output.md`를 작성합니다.
...
이 방식은 Python runner가 결과를 상상해서 만드는 것이 아닙니다. Claude Code가 command md와 fixture를 읽고 만든 실제 결과 파일을 runner가 검증합니다.
...
| `loaded` | 중간 | 파일 경로, sha256, Rule ID가 남음 | Claude가 `mark-loaded-file.sh`를 호출해야 함 |
| `evidence` | 중간 | `file_path#RULE-ID`로 근거를 명시 | Claude가 작성한 설명이므로 자기보고 성격이 있음 |
...
| Replay claude_mediated 모드 | 중간~높음 | Claude Code가 command md와 fixture로 실제 결과를 새로 작성 | 완전한 비대화 자동 실행은 아님 |
...
| 개선 작업 | 효과 |
|-----------|------|
| Claude CLI/API 기반 replay 실행기 | replay를 사람이 중간에 수행하지 않고 더 재현 가능하게 실행 |
| 실제 파일 read hook 연동 | Claude가 어떤 파일을 열었는지 더 직접적으로 수집 |
...
1. Claude CLI/API 기반으로 replay를 완전 자동화하는 것
...
그 다음 Claude Code에서 아래 흐름을 사용합니다.
